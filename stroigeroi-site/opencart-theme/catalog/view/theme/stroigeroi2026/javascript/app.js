...
(function () {
  ...
  /* ======================================================================
     Видимый фокус на карте магазина

     По сайту фокус — красное кольцо, и его рисует общее правило
     :focus-visible. До кадра с картой оно не доходит. Причина не в стилях:
     когда Tab уводит фокус в кадр, фокус оказывается ВНУТРИ чужого
     документа, и для родительской страницы элемент-кадр не сфокусирован —
     ни :focus, ни :focus-visible, ни :focus-within на нём не срабатывают.
     Событие focus на самом кадре тоже не приходит.

     Единственный надёжный признак — окно теряет фокус, а активным
     элементом оказывается кадр. По нему и ставим класс.

     Без этого человек с клавиатуры доходит до карты и перестаёт понимать,
     где он: кольцо есть у всего, кроме неё.
     ====================================================================== */
  var mapFrames = $$('.shop-card__mapframe');

  if (mapFrames.length) {
    var paintMapFocus = function () {
      var active = document.activeElement;
      mapFrames.forEach(function (frame) {
        frame.classList.toggle('is-focus', frame === active);
      });
    };
    /* Перерисовываем при каждом событии, после которого фокус мог
       поменяться: уход из окна (в кадр), возврат в окно и любое
       перемещение фокуса внутри самой страницы. */
    window.addEventListener('blur', paintMapFocus);
    window.addEventListener('focus', paintMapFocus);
    document.addEventListener('focusin', paintMapFocus);
  }
  ...
  /* ======================================================================
     Календарь движка (bootstrap-datetimepicker 3.1.3 из OpenCart): поле
     «Дата заказа» на странице возврата, опции товара с датой. Шаблоны
     движка запускают его с языком из языкового пакета, и на сайте он
     вышел по-английски - September, Mo Tu We. Названия месяцев и дней
     виджет рисует один раз, при запуске, поэтому язык не поправить
     ни стилями, ни настройкой потом: календарь запускается заново,
     с теми же настройками, только по-русски. Кнопке со значком -
     подпись для экранного диктора: в ней ничего, кроме значка.
     Вид окна - opencart.css. В макете jQuery нет - здесь ничего
     не делается.
     ====================================================================== */
  (function () {
    var jq = window.jQuery;
    var mom = window.moment;
    if (!jq || !jq.fn.datetimepicker || !mom || !mom.locales || mom.locales().indexOf('ru') < 0) return;
    jq(function () {
      jq('.date, .datetime, .time').each(function () {
        var el = jq(this);
        var dp = el.data('DateTimePicker');
        if (!dp) return;
        el.find('.btn').each(function () {
          if (!this.getAttribute('aria-label') && !this.textContent.trim()) {
            this.setAttribute('aria-label', 'Открыть календарь');
          }
        });
        if (dp.options.language === 'ru') return;
        var opts = jq.extend({}, dp.options, { language: 'ru' });
        dp.destroy();
