...
# Номера телефонов написаны в шаблонах словом, в двух десятках мест,
# и ещё в app.js. 24.09.2026 основной номер сменился, а у каждого
# магазина появились свои номера (по карточкам 2ГИС). Поменять номер
# не везде - значит оставить покупателю ссылку, по которой звонок уйдёт
# не туда. Поэтому все ссылки tel: в теме сверяются со списком
# действующих номеров: номер не из списка - опечатка или старый номер.
# Сменился номер - правится этот список, и проверка покажет каждое
# место со старым.
PHONES = {
    '+79638319999': 'основной; ул. Чубарова, 16 к40',
    '+74152319999': 'ул. Чубарова, 16 к40, городской',
    '+79638300999': 'просп. 50 лет Октября, 17А',
    '+74152400999': 'просп. 50 лет Октября, 17А, городской',
    '+79638300333': 'Елизово, Магистральная ул., 2/2',
    '+74152400333': 'Елизово, Магистральная ул., 2/2, городской',
...
for num, where in sorted(tels.items()):
    if num not in PHONES:
        print(f'tel:{num} не из списка действующих номеров — {", ".join(sorted(where))}'); bad += 1
for num, what in PHONES.items():
    if num not in tels:
        print(f'Номер {num} ({what}) в списке есть, а на сайте его нет — убрать из списка или вернуть на сайт'); bad += 1
print(f'Номеров в ссылках tel: {len(tels)}, все из списка действующих' if set(tels) <= set(PHONES) else '')
