...
$cases = [
    ...
    // Знак валюты с пробелом-текстом «&nbsp;»: так он записан в настройках
    // живого магазина - фильтр OCFilter показывает его буквально,
    // «13776&nbsp;руб». Без этих строк цена не распознавалась совсем.
    ['4 320,00&nbsp;руб',    '',              '5',             '4320.00', 'InStock'],
    ['4&nbsp;320,00&nbsp;руб', '',            '5',             '4320.00', 'InStock'],
...
foreach ($cases as $i => [$price, $special, $stock, $want_price, $want_avail]) {
    ...
    if ($want_price !== null) {
        ...
        // Цифры - с того, что видит покупатель: «&#160;» на странице - пробел,
        // а не число 160.
        $digits = preg_replace('~\D~', '', html_entity_decode($shown, ENT_QUOTES | ENT_HTML5, 'UTF-8'));
...
foreach ($m[1] as $i => $json) {
    ...
    foreach ($d['department'] ?? [] as $dep) {
        ...
        // С 24.09.2026 у каждого магазина свой номер; в разметке он обязан
        // быть тем же, что в списке «Магазины» на странице.
        $tel = '+' . preg_replace('/\D/', '', $dep['telephone'] ?? '');
        if ($tel === '+' || strpos($page, 'href="tel:' . $tel . '"') === false) {
            echo "header.twig: у точки «{$street}» номер в разметке ({$tel}) не совпадает ни с одной ссылкой на странице\n";
            $bad++;
        }
    }
}
...
$wantSub = [
    ...
    'ЭЛЕКТРОИНСТРУМЕНТЫ' => 'plugzap', 'Электроинструменты' => 'plugzap',
    'Краскопульты и пневмоинструмент' => 'spray',
