...
«Disallow: /index.php?route=». До 23.09.2026 человекопонятные адреса
на stroigeroi.ru были выключены, каждая страница жила по index.php?route=...,
и эта одна строка убрала бы из поиска всё. Теперь каталог на коротких
адресах, но по index.php?route=... по-прежнему живут карта сайта, корзина,
контакты и прежние адреса каталога, которые поисковики уже знают. Глазами
такое пропускают, потому что правило выглядит разумным.
...
def main():
    ...
    for p in wide:
        if p.rstrip('*') == '/':
            print(f'Правило «Disallow: {p}» закрывает весь сайт')
        else:
            print(f'Правило «Disallow: {p}» закрывает карту сайта, корзину, контакты '
                  f'и все прежние адреса каталога: они живут по index.php?route=... '
                  f'(а до 23.09.2026, пока ЧПУ были выключены, — весь сайт)')
        bad += 1
