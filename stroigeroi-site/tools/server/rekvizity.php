...
/*
  ...
  под ней, и то и другое печатается в отчёте. Кроме трёх надписей
  «Поделиться», «Скачать», «Распечатать»: на живом сайте они нашлись
  именно так, простым текстом без ссылок (подписи кнопок, скопированные
  вместе с реквизитами), и заказчик попросил их убрать (правка 26,
  rekvizity-ubrat.php). Здесь они пропускаются, чтобы при повторной
  установке страницы не вернулись.
...
if ($dropped) {
    echo 'надписи без ссылок, убраны: «', implode('», «', $dropped), "»\n";
}
foreach ($extra as $line) {
