...
class TestParser(unittest.TestCase):
    ...
    def test_name_split_across_two_lines(self):
        """Наименование, перенесённое на две строки, собирается обратно.

        Реальная карточка: «Общество с ограниченной ответственностью» одной
        строкой, ««ЮРЛИДЕР»» следующей. По отдельности ни одна строка
        наименованием не выглядит, и раньше название не находилось вовсе.
        """
        parsed = parse_text(
            "Общество с ограниченной ответственностью\n"
            "«ЮРЛИДЕР»\n"
            "Юридический адрес: 191015, г.Санкт-Петербург, пер.Калужский, д.3\n"
            "ИНН: 7806298843\nКПП: 784201001\nОГРН: 1187847048476\n"
            "Банк: Санкт-Петербургский филиал ОАО «Промсвязьбанк»\n")
        self.assertEqual(parsed.company.full_name,
                         "Общество с ограниченной ответственностью «ЮРЛИДЕР»")
        # Банк не должен подменить наименование компании.
        self.assertNotIn("Промсвязьбанк", parsed.company.full_name)
        self.assertNotIn("Промсвязьбанк", parsed.company.short_name)

        # Краткая форма с переносом — тоже.
    ...
    def test_label_without_colon_and_wrapped_value(self):
        """Карточка-«простыня»: подпись и значение через пробел, без двоеточия.

        Так печатают карточки предприятия в PDF: «Юридический адрес 194363,
        г. Санкт-Петербург, вн. тер. г.» — и продолжение адреса на следующих
        строках. Раньше адреса не находились совсем.
        """
        parsed = parse_text(
            "КАРТОЧКА ПРЕДПРИЯТИЯ\n"
            "Полное наименование  ООО «АльфаСтройКомплекс Северо-Запад»\n"
            "Краткое наименование ООО «АСК Северо-Запад»\n"
            "Юридический адрес 194363, г. Санкт-Петербург, вн. тер. г.\n"
            "Поселок Парголово, ул. Николая Рубцова, д.\n"
            "9 литера А, кв.46\n"
            "Фактический адрес 194363, г. Санкт-Петербург, вн. тер. г.\n"
            "Поселок Парголово, ул. Николая Рубцова, д.\n"
            "9 литера А, кв.46\n"
            "Банк ООО «Банк Точка»\n"
            "ИНН/КПП ИНН 7802964323, КПП 780201001\n"
            "ОГРН 1257800039012\n")
        company = parsed.company

        # Адрес собран из трёх строк.
        for field_name in ("legal_address", "actual_address"):
            with self.subTest(field=field_name):
                value = company.get(field_name)
                self.assertIn("Поселок Парголово", value)
                self.assertIn("кв.46", value)

        # Краткое наименование — именно краткое, а не полное.
        self.assertEqual(company.short_name, "ООО «АСК Северо-Запад»")
        # Банк наименованием компании не стал.
        self.assertNotIn("Точка", company.full_name)
        self.assertNotIn("Точка", company.short_name)

    def test_ids_do_not_span_lines(self):
        """ИНН, КПП и ОГРН не должны «съезжать» на следующую строку.

        В карточке «КПП 780201001» стоит в конце строки, а ниже начинается
        «ОГРН 1257800039012». Раньше КПП получался из 11 цифр — цифра
        следующей строки прилипала к нему.
        """
        parsed = parse_text("ИНН 7802964323, КПП 780201001\nОГРН 1257800039012\n")
        self.assertEqual(parsed.company.kpp, "780201001")
        self.assertEqual(parsed.company.inn, "7802964323")
        self.assertEqual(parsed.company.ogrn, "1257800039012")

    def test_two_positions_in_one_table_row(self):
        """Одна строка таблицы с двумя подписями и двумя фамилиями.

        В учётных карточках встречается строка, где слева в одной ячейке
        «Генеральный директор» и «Главный бухгалтер», а справа — две
        фамилии. Целиком это не читается, и руководитель терялся.
        """
        from src.company_parser import expand_multiline_pairs

        pairs = [("Генеральный директор\n\nГлавный бухгалтер",
                  "Петрушкин Александр Николаевич\n\nПетрушкин Александр Николаевич")]
        self.assertEqual(
            expand_multiline_pairs(pairs),
            [("Генеральный директор", "Петрушкин Александр Николаевич"),
             ("Главный бухгалтер", "Петрушкин Александр Николаевич")])

        # Когда подписей и значений НЕ поровну, строку не трогаем: слева
        # «Банковский счет», справа четыре строки с БИК, К/С и Р/С.
    ...
    def test_numbered_card_layout(self):
        """Нумерованная «КАРТОЧКА ПРЕДПРИЯТИЯ»: строки начинаются с номера.

        Три особенности такой карточки: перед подписью стоит номер строки
        таблицы, ОГРН написан словами без сокращения, а длинное название
        разорвано переносом по дефису.
        """
        parsed = parse_text(
            "КАРТОЧКА ПРЕДПРИЯТИЯ\n"
            "1\n"
            "Организационно-правовая форма\n"
            "предприятия. Полное наименование на\n"
            "русском языке\n"
            "Общество с ограниченной ответственностью «Строительно-\n"
            "монтажное управление 61»\n"
            "2 Сокращенное наименование на русском\n"
            "языке ООО «СМУ61»\n"
            "4\n"
            "Основной государственный\n"
            "регистрационный номер\n"
            "(в едином гос. реестре юр. лиц)\n"
            "1256100027677\n"
            "5 Юридический адрес 344029, Ростовская область, Ростов-на-Дону, ул,\n"
            "102/2, 5, офис 4\n"
            "11 Идентификационный номер\n"
            "налогоплательщика (ИНН) 6166133729\n")
        company = parsed.company

        # Перенос по дефису склеен без пробела.
        self.assertEqual(
            company.full_name,
            "Общество с ограниченной ответственностью «Строительно-монтажное управление 61»")
        self.assertEqual(company.short_name, "ООО «СМУ61»")

        # ОГРН найден, хотя сокращения «ОГРН» в карточке нет.
        self.assertEqual(company.ogrn, "1256100027677")
        self.assertEqual(company.inn, "6166133729")

        # Номер строки таблицы не помешал опознать подпись адреса.
        self.assertIn("Ростовская область", company.legal_address)
        self.assertIn("офис 4", company.legal_address)

    def test_wrapped_label_is_not_taken_as_value(self):
        """Хвост перенесённой подписи не должен попасть в значение.

        «3 Полное наименование на английском» — здесь «на английском» это
        продолжение подписи, а не наименование компании.
        """
        parsed = parse_text("3 Полное наименование на английском\n"
                            "языке LIMITED LIABILITY COMPANY\n"
                            "ИНН 7812345675\n")
        self.assertNotEqual(parsed.company.full_name, "на английском")
