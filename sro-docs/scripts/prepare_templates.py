...
def op_font(paragraph: int, node: int, like_node: int,
            like_paragraph: int | None = None) -> dict:
    """Дать фрагменту такое же оформление, как у другого фрагмента.
    ...
    По умолчанию образец берётся из ТОГО ЖЕ абзаца. `like_paragraph` позволяет
    взять его из соседнего: так лечатся пустые линейки бланка, у которых от
    прежних правок остался чужой стиль (например, заголовочный — крупный
    полужирный), незаметный, пока линейка пуста, и бросающийся в глаза,
    как только в неё подставлено наименование.
    """
    return {"kind": "font", "p": paragraph, "n": node, "like": like_node,
            "like_p": like_paragraph}
...
# ============================================================ ЗАЯВЛЕНИЕ ОРС
# Бланк Ассоциации «Объединение Ростовских строителей». Две особенности,
# каких нет у других СРО:
#   * ОГРН юридического лица и ОГРНИП предпринимателя — РАЗНЫЕ строки
#     клеток. Заполняется та, что соответствует заявителю, вторая
#     остаётся пустой (у юрлица ОГРНИП не бывает, и наоборот);
#   * у фонда возмещения вреда шесть вариантов: пять уровней и «Простой
#     (снос)» отдельной строкой.
# Наименование пишется на одну из двух линеек: юрлицо — на верхнюю
# (под ней подпись «правовая форма…»), предприниматель — на нижнюю
# (под ней «Фамилия, Имя, Отчество ИП»).
ORS_APPLICATION_OPS: list[dict] = [
    # --- наименование: своя линейка для юрлица и для предпринимателя ---
    op_text(10, 0, "                      ", "{{name_for_company}}"),
    # У этой линейки в бланке остался заголовочный стиль (крупный полужирный
    # Arial): пока она пуста, этого не видно, а с наименованием строка резко
    # выбивается из документа. Берём оформление соседней линейки для ИП —
    # Times New Roman 12, как весь бланк.
    op_font(10, 0, 0, like_paragraph=13),
    op_text(13, 0, ".", ".{{name_for_entrepreneur}}"),

    # --- адреса ---
    op_text(15, 0, "адрес юридического лица /адрес регистрации по месту жительства   ",
            "адрес юридического лица /адрес регистрации по месту жительства  "
            "{{legal_address}}   "),
    op_text(17, 0, "почтовый адрес: принять в члены ",
            "почтовый адрес: {{postal_address}} принять в члены "),

    # --- ИНН: 12 клеток (клетка 0 — подпись «ИНН») ---
    *[op_cell(0, 0, i + 1, "", "{{inn_d%d}}" % (i + 1)) for i in range(12)],

    # --- ОГРН юрлица: 13 клеток ---
    *[op_cell(1, 0, i + 1, "", "{{ogrn_ul_d%d}}" % (i + 1)) for i in range(13)],

    # --- ОГРНИП предпринимателя: 15 клеток ---
    *[op_cell(2, 0, i + 1, "", "{{ogrnip_d%d}}" % (i + 1)) for i in range(15)],

    # --- телефон и почта ---
    op_text(68, 0, "Телефон:   ", "Телефон: {{phone}}   "),
    op_text(71, 4, "): ", "): {{email}}"),

    # --- виды объектов (таблица №3, вторая строка) ---
    op_mark(3, 1, 0, "V", "{{mark_object_ordinary}}", size=32),
    op_mark(3, 1, 1, "", "{{mark_object_hazardous}}", size=32),
    op_mark(3, 1, 2, "", "{{mark_object_nuclear}}", size=32),

    # --- уровни возмещения вреда: пять уровней и «Простой (снос)» ---
    op_mark(4, 1, 3, "V", "{{mark_harm_level1}}", size=36),
    *[op_mark(4, n, 3, "", "{{mark_harm_level%d}}" % n, size=36)
      for n in (2, 3, 4, 5)],
    op_mark(4, 6, 3, "", "{{mark_harm_level6}}", size=36),

    # --- уровни договорных обязательств: пять ---
    *[op_mark(5, n, 3, "", "{{mark_contract_level%d}}" % n, size=36)
      for n in (1, 2, 3, 4, 5)],

    # --- подпись: должность и фамилия разнесены табуляцией ---
    # В бланке это один абзац «<таб><таб>.» над тремя подписями строки ниже:
    # «(должность)», «(подпись)», «(фамилия и инициалы)». У абзаца заданы свои
    # позиции табуляции — 385 (первый столбец) и 6751 (третий). Ставим ровно
    # две табуляции, как в бланке: должность встанет над «(должность)»,
    # фамилия — над «(фамилия и инициалы)», а место для подписи между ними
    # останется пустым.
    op_regex(149, r"^\t\t\.$",
             "\t{{director_position}}\t{{director_short_name}}"),
    # Та же беда, что и у линейки наименования: строка подписи набрана
    # Helvetica 10,5 серым, хотя подписи под ней — Times New Roman.
    op_font(149, 0, 0, like_paragraph=13),
]


# ============================================================ ДОВЕРЕННОСТЬ ОРС
# Бланк прислан ЗАПОЛНЕННЫМ образцом: в нём стоят «ООО ХХХХХХХ» вместо
# доверителя и «ХХХХХХХХХ.» вместо подписи. Все они перечислены здесь
# в «ожидаемом тексте»: если бланк изменится, разметка остановится.
# Сведения о представителе (Соколова О.В.) берутся из sro/ОРС/attorney.json.
ORS_POWER_OPS: list[dict] = [
    # --- номер доверенности ---
    op_text(3, 0, "ДОВЕРЕННОСТЬ №1", "ДОВЕРЕННОСТЬ № {{power_number}}"),

    # --- место и дата составления ---
    op_regex(5, r"^Ростов-на-Дону\.\s+15\.09\.2026 г\.$",
             "г. {{sro_city}}\t{{doc_date}} г.", tab_pos=9355),

    # --- преамбула: кто выдаёт доверенность и кому ---
    op_text(7, 1,
            "ООО ХХХХХХХ настоящей доверенностью уполномочивает гражданина РФ "
            "Соколову Ольгу Викторовну, паспорт серия 60 24 №041103, выдан "
            "15.07.2023 ГУ МВД России по Ростовской области, зарегистрирована "
            "по адресу: Ростовская обл,, гор. Ростов-на-Дону, переулок "
            "Измаильский, дом 39А кв 24, представлять интересы Доверителя, "
            "как члена СРО Ассоциации «Объединение Ростовских строителей» "
            "(далее – «Ассоциация») и голосовать от имени Доверителя, "
            "на очередном и внеочередном Общем собрании членов Ассоциации. "
            "С правом принимать участие в обсуждении вопросов повестки Общего "
            "собрания членов Ассоциации, подписывать документы, касающиеся "
            "вопросов повестки Общего собрания и голосовать по ним от имени "
            "Доверителя.",
            "{{company_full_display}} настоящей доверенностью уполномочивает "
            "гражданина РФ {{attorney_full_name_genitive}}, паспорт "
            "{{attorney_passport}}, выдан {{attorney_passport_date}} "
            "{{attorney_passport_issued_by}}, {{attorney_registered_word}} "
            "по адресу: {{attorney_reg_address}}, представлять интересы "
            "Доверителя, как члена СРО {{sro_name}} "
            "(далее – «Ассоциация») и голосовать от имени Доверителя, "
            "на очередном и внеочередном Общем собрании членов Ассоциации. "
            "С правом принимать участие в обсуждении вопросов повестки Общего "
            "собрания членов Ассоциации, подписывать документы, касающиеся "
    ...
    op_text(9, 0, "Подпись______________________/Соколова О.В./ удостоверяю.",
            "Подпись______________________/{{attorney_short_name}}/ удостоверяю."),

    # --- подпись руководителя ---
    op_text(12, 0, "Директор                 _________________      /",
            "{{director_position}}                 _________________      /"),
    op_text(12, 2, "ХХХХХХХХХ./", "{{director_short_name}}/"),
]
...
TEMPLATES: dict[str, list[tuple]] = {
    ...
    "ОРС": [
        ("Заявление_ОРИГИНАЛ.docx", "01_Заявление.docx", ORS_APPLICATION_OPS),
        ("Доверенность_ОРИГИНАЛ.docx", "02_Доверенность.docx", ORS_POWER_OPS),
    ],
    "ЯРД": [
...
#: Настройки абзаца и фрагмента. Внутрь заглядывать нельзя: там лежат
#: <w:tab/> совсем другого смысла — не символ табуляции в тексте, а ПОЗИЦИЯ
#: табуляции (куда встаёт курсор). Если их спутать, текст абзаца читается
#: с лишними отступами, а разметка стирает разметку бланка.
_SETTINGS_TAGS = (W + "pPr", W + "rPr")


def _own_tabs(paragraph) -> list:
    """Символы табуляции этого абзаца, НЕ считая вложенных абзацев.

    Позиции табуляции из настроек абзаца (<w:pPr><w:tabs>) сюда не попадают:
    это разметка бланка, её трогать нельзя.
    """
    found: list = []

    def walk(node) -> None:
        for child in node:
            if child.tag == W + "p" or child.tag in _SETTINGS_TAGS:
                continue
            if child.tag == W + "tab":
                found.append(child)
            else:
...
def apply_ops(document_xml: bytes, ops: list[dict], label: str) -> bytes:
    ...
    for op in ops:
        ...
        if op["kind"] == "cells":
            ...
            nodes = _own_text_nodes(paragraphs[op["p"]])
            like_p = op["p"] if op.get("like_p") is None else op["like_p"]
            like_nodes = (nodes if like_p == op["p"]
                          else _own_text_nodes(paragraphs[like_p]))
            source = like_nodes[op["like"]].getparent().find(W + "rPr")
            if source is None:
                raise PrepareError(
                    f"{label}: у фрагмента №{op['like']} абзаца №{like_p} нет "
