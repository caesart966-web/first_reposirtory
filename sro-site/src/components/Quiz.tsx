...
// Фон тёмной секции заявки: рабочий стол сверху — чертёж со штампом, рулон,
// каска, блокнот, ручка, линейка. Сцена нарисована, scripts/make-desk.py.
//
// Заказчик показал макет именно с этой сценой. Фотографии под неё нет и взять
// негде: фотостоки закрыты егресс-политикой среды. Поэтому нарисовано — и
// сверху, а не с угла как на макете: вид сверху убирает светотень, а без неё
// построение не выдаёт себя.
//
...
// Фонового кадра под квизом сейчас НЕТ — намеренно. Здесь стояли по очереди
// кадр стола (ушёл на первый экран: одна сцена открывала и закрывала
// страницу) и архивная синька (заказчик отклонил). Новый кадр заказчик
// пришлёт через репозиторий — скачать из интернета из этой среды нельзя,
// фотостоки закрыты сетевой политикой. Как появится: <picture> с кадром на
// opacity-50 плюс плёнка bg-accent-950/65 — обе величины меряются вместе,
// см. README «Фон секции заявки».
...
export function Quiz() {
  ...
  return (
    ...
    <Section id="quiz" size="key" className="relative overflow-hidden bg-accent-950">
      {/* Два мягких пятна фирменного цвета: без них тёмная заливка во всю
          ширину читалась плоской плашкой. Это временная замена кадру. */}
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        <div className="absolute -top-24 right-[-8%] h-[420px] w-[420px] rounded-full bg-accent-700/30 blur-3xl" />
        <div className="absolute bottom-[-25%] left-[-6%] h-[380px] w-[380px] rounded-full bg-accent-800/40 blur-3xl" />
      </div>
