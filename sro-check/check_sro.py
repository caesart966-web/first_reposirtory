...
def check_company(
    ...
    for source in excel_io.registry_order(priority):
        ...
        # Останавливаемся, только когда ответ уже окончательно «да»: по этому
        # реестру видно действующее членство, и второй ничего не добавит.
        # А вот если запись есть, но недействующая (исключён/приостановлен),
        # второй реестр проверить ОБЯЗАТЕЛЬНО — компания может действующе
        # состоять именно в нём. Иначе исключение из одного реестра давало бы
        # ложное «нет», хотя в другом членство в силе.
        if not check_all and result.verdict is Verdict.YES:
            break
