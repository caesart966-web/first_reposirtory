...
ROLE_PATTERNS: tuple[RolePattern, ...] = (
    ...
    # ------------------------------ телефон -------------------------------- #
    RolePattern(ROLE_PHONE, r"(^|\W)(телефон|тел|тлф|моб|мобильный|факс)(\W|$)", 11, None),
    RolePattern(ROLE_PHONE, r"номер телефона|контактный телефон|телефоны", 11, None),

    # -------------------------------- email -------------------------------- #
    RolePattern(ROLE_EMAIL, r"e ?mail|электронн(ая|ой) почт|эл почт|адрес электронной почты", 12, None),
    ...
    RolePattern(
        ...
        10,
        r"электронн|сайт|e ?mail",
    ),
    RolePattern(ROLE_ADDRESS, r"(^|\W)адрес(\W|$)", 9, r"электронн|сайт|e ?mail|ip"),
    ...
    # ------------------------------- сайт ---------------------------------- #
    RolePattern(ROLE_WEBSITE, r"(сайт|web|www|url|интернет ресурс)", 9, None),
