...
// письма, поэтому при сбое почты ничего не теряется.
//
// РАСШИРЕНИЕ .php ЗДЕСЬ НЕ ОПЕЧАТКА. В журнале лежат имена, телефоны
// и сообщения посетителей — персональные данные, которые нельзя отдавать
// по прямой ссылке. Раньше файл назывался leads.jsonl и закрывался
// файлом .htaccess. Беда в том, что .htaccess действует только на Apache,
// а половина российских хостингов отдаёт статику через nginx и .htaccess
// просто не читает: там журнал открывался бы по адресу /api/leads.jsonl
// любому, кто его наберёт.
//
// Теперь защита не зависит от типа хостинга: первой строкой файла пишется
// открывающий тег PHP с командой exit (её видно в функции log_lead ниже),
// и при попытке открыть журнал в браузере PHP выполняет exit и не отдаёт
// ни строки. Дописывать в такой файл ничего не мешает.
// .htaccess остался как второй рубеж для Apache.
//
// В комментариях этого файла закрывающего тега PHP быть не должно:
// однострочный комментарий обрывается не только переносом строки,
// но и этим тегом — и всё, что стоит после него, сервер отдаёт
// посетителю как обычный текст. На этом уже один раз попались:
// сама фраза с примером тега выдала кусок исходника в ответ на заявку.
const LEADS_LOG = __DIR__ . '/leads.log.php';
...
// Без «: never» в объявлении нарочно: этот тип появился в PHP 8.1,
// и на хостинге с 8.0 файл не выполнился бы вовсе — не «форма не работает»,
// а белый экран с ошибкой разбора. Остальной код обходится возможностями
// PHP 7.4, то есть заведомо любым живым хостингом.
...
function deliver(string $to, string $subject, string $body, array $headers): bool
{
    ...
    //
    // Адрес проверяется, хотя он и задан константой: строка отсюда уходит
    // в командную строку почтовой программы, и опечатка с пробелом или
    // кавычкой превратится не в «письмо не дошло», а в неожиданный параметр.
    $envelope = filter_var(MAIL_FROM, FILTER_VALIDATE_EMAIL);
    if ($envelope === false) {
