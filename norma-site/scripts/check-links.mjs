...
//  5. повторяющиеся id на одной странице — якорь ведёт только к первому.
//
...
for (const file of files) {
  ...
  // Тире в наборе — всегда с пробелами по обе стороны, поэтому «—<a»
  // и «</a>—» означают съеденный перенос строки, а не задумку. Дефис
  // в класс НЕ входит: «СРО-<b>партнёр</b>» — законная запись.
  // Четыре таких случая нашлись сразу и жили незамеченными: прежний
  // класс символов знал про буквы и точку, а про тире — нет.
  ...
  // Повторяющиеся id. Якорь ведёт к первому совпадению, поэтому вторая
  // секция с тем же id просто недостижима по ссылке, и заметить это глазами
  // почти невозможно — страница выглядит целой.
  const ids = [...html.matchAll(/\sid="([^"]+)"/g)].map((m) => m[1])
  const seen = new Set()
  for (const id of ids) {
    if (seen.has(id)) problems.push(`${pageUrl}: id="${id}" встречается дважды — якорь ведёт только к первому`)
    seen.add(id)
  }
...
console.log('\n✓ Битых ссылок и заглушек нет, мета-теги и id уникальны, слова не слиплись.')
