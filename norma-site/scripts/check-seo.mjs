...
if (entries.size > 0) {
  ...
  for (const [loc, e] of entries) {
    ...
    // Дата изменения: у статей она берётся из их собственного поля updated
    // и есть всегда, у остальных страниц — из истории git, и на мелкой
    // копии репозитория (fetch-depth по умолчанию) её взять неоткуда.
    // Поэтому у статьи отсутствие даты — ошибка, у страницы — замечание:
    // проверка не должна падать из-за того, как склонировали репозиторий.
    if (!e.lastmod) {
      const path = new URL(loc).pathname
      if (/\/baza-znaniy\/[^/]+\/$/.test(path)) bad(`в карте сайта нет даты изменения у статьи ${path}`)
      else warn(`в карте сайта нет даты изменения у ${path} — репозиторий склонирован без истории git`)
    }
