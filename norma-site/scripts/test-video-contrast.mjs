// Проверяет, что текст поверх карты на первом экране читается.
//
// Зачем отдельная проверка. Обычные инструменты доступности считают контраст
// по цвету фона в стилях — и на первом экране видят только тёмно-синий цвет
// секции. А под текстом там не цвет, а движущаяся карта России, и на светлом
// её месте надпись может пропасть, хотя формально всё в порядке. Поэтому
// здесь смотрим настоящие пиксели на шести секундах ролика и берём самый
// светлый — то есть худший — случай.
//
// Что эта проверка уже поймала: строку «2 минуты · 4 вопроса» приглушённым
// серым — 3.38:1 при норме 4.5. Она попадает ровно на скопление огней вокруг
// Москвы, самое светлое место карты, и глазами это выглядит нормально.
//
...
const MOMENTS = [0.3, 2.0, 3.6, 4.3, 6.0, 8.0] // секунды ролика
...
const PAGES = [
  {
    url: '/',
    section: '.hero',
    texts: ['.geo', '.page-title', '.tagline', '.note', '.stat .v', '.stat .d'],
    // Значки, чипы норм и белая карточка предложения — у них своя
    // подложка, мерить по ним фон бессмысленно.
    hide: '.hero svg, .hero .hero-offer, .hero .law, .hero .cta',
  },
]

// Окно телефона нарочно высокое. Замер берётся с видимой области экрана,
// а на телефоне первый экран длиннее любого настоящего телефона: при высоте
// 800 три нижние строки просто не попадали в кадр и молча пропускались.
const SCREENS = [
  [1280, 900, 'компьютер'],
  [390, 1500, 'телефон'],
]
...
for (const page_ of PAGES) {
  for (const [width, height, screen] of SCREENS) {
    ...
    await page.addStyleTag({
      content: `${page_.section}, ${page_.section} * {
