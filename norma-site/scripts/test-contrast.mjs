...
// Пропускать больше нечего. Раньше здесь стоял первый экран: под его текстом
// шло видео, и расчёт по цвету фона врал в обе стороны — он видел только цвет
// секции и не замечал ни кадра, ни пелены поверх него. Видео с сайта убрано,
// первый экран лежит на белой бумаге и проверяется наравне со всем остальным.
const SKIP_INSIDE = []
...
if (unknown.size) {
  ...
  console.log('  Это подложки-градиенты у карточек — цвет под текстом там меняется по ширине.')
}

if (failed) {
  console.log(`\nОШИБОК: ${failed}. Текст читается хуже нормы.
Частая причина: тёмная секция или шапка страницы без класса section--dark
или page-head — тогда выделенное слово, чип нормы или штамп «Проверено»
