...
// Пропускать больше нечего. Раньше здесь стоял первый экран: под его
// текстом шло видео, и расчёт по цвету фона врал в обе стороны. Теперь
// видео живёт отдельной полосой без единой надписи, а текст первого экрана
// лежит на сплошном цвете — то есть проверяется наравне со всем остальным.
// Что на полосе с картой действительно нет текста, стережёт отдельная
// проверка: scripts/test-video-contrast.mjs.
const SKIP_INSIDE = []
