...
const HEADS = [
  ...
  '/kontakty/',
  '/poisk/',
  // Города со своими фотографиями. Шапка города рисует кадр, только если
  // файл действительно лежит в public/img/ — города без файла проверка
  // пропускает с пометкой «фотографии нет», а не падает: снимки заказчик
  // присылает по одному, и ждать все тридцать три было бы нечем.
  '/sro/moskva/',
  '/sro/sankt-peterburg/',
  '/sro/rostov-na-donu/',
