...
const GRADES = {
  ...
  karta: {
    filter: [
      // curves поднимает сушу и огни городов, но НЕ трогает море.
      // Это здесь главное. Карта лежит фоном под белым текстом, и поверх
      // неё идёт пелена: если поднять кадр целиком (гаммой или яркостью),
      // вместе с сушей посветлеет и море — а именно на море лежит бо́льшая
      // часть букв, и они перестанут читаться. Точки кривой подобраны так:
      // тёмное (море, 0.10) остаётся тёмным, средние тона (суша, 0.30)
      // подскакивают почти вдвое, светлое (огни городов) идёт в максимум.
      "curves=all='0/0 0.12/0.10 0.3/0.45 0.6/0.85 1/1'",
      'eq=contrast=1.05:saturation=1.22',
      'vignette=angle=PI/7.5',
