...
// Фотография в шапке города показывается, только если файл реально лежит
// в public/img/ — так заказчик может положить снимок без правки кода.
// Обратная сторона: имя, вписанное в regions.ts под файл, которого ещё нет,
// не ломает ничего и потому теряется навсегда. Шапка просто остаётся
// ровно тёмной, и заметить это можно лишь случайно. Напоминаем.
const before2b = problems
console.log('\nФотографии городов')
const { REGIONS } = await import('../src/config/regions.ts')
const waitingPhoto = REGIONS.filter((r) => r.photo && !existsSync(join('public', 'img', r.photo)))
for (const r of waitingPhoto) {
  console.log(`  · ${r.city}: в regions.ts вписан ${r.photo}, а файла в public/img/ нет — шапка осталась тёмной`)
}
if (problems === before2b && waitingPhoto.length === 0) {
  const withPhoto = REGIONS.filter((r) => r.photo).length
  console.log(`  ✓ все обещанные фотографии на месте (${withPhoto})`)
}
