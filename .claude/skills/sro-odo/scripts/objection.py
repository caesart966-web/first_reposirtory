...
def t_customer(f):
    ...
    if ck in SUBCONTRACT:
        extra = " Договор заключён на торгах: по букве ч. 3 ст. 55.8 он конкурентный, но по письму Минстроя № 18965-ОС/02 обязательства перед СРО возникают только там, где членство требуется законом." if f.get("procurement") == "competitive" else ""
        return "founded", "Заказчик по договору — не застройщик, не технический заказчик, не эксплуатант и не региональный оператор: это субподряд, членства не требует и в совокупный размер не входит." + b + extra, ["grk-52-2.1", "grk-48-4", "grk-47-2", "minstroy-18965"]
    if ck == "developer":
...
def t_threshold(f):
    ...
    if k in ("design", "survey"):
        return "unfounded", "Для проектирования и изысканий суммового порога нет: договор с застройщиком или техзаказчиком требует членства при любой цене.", ["grk-48-4", "grk-47-2"]
    ...
    if p <= thr:
        return "founded", f"Размер обязательств {fmt(p)} ₽ не превышает {fmt(thr)} ₽: обязательного членства по этому договору нет, и в совокупный размер по КФ ОДО он не включается, даже если заключён на торгах (разъяснение НОСТРОЙ от 11.05.2022 к 124-ФЗ, письмо Минстроя № 18965-ОС/02). Если один объём работ разбит на несколько договоров ниже порога, СРО вправе оценить это как обход.", ["grk-52-2.1" if k == "construction" else "grk-55.31-5", "fz-124", "nostroy-2022-05-11", "minstroy-18965"]
...
def t_competitive(f):
    ...
    if pr == "direct":
        ...
        return "unfounded", "По практике этой СРО совокупный размер считается по всем договорам подряда с застройщиком и техзаказчиком: прямой договор входит.", ["grk-55.8-4", "fz-309"]
    return "needs_facts", "Способ заключения договора не установлен: нужны протокол торгов, извещение или письмо заказчика.", ["grk-55.8-3"]


def t_executed(f):
    if f.get("has_final_act"):
        return "founded", "Результат принят по акту: обязательства, признанные исполненными на основании акта приёмки, в фактический совокупный размер не включаются. Об исполнении подаётся уведомление в СРО в три рабочих дня.", ["grk-55.13-7", "gk-753", "grk-55.8-4", "minstroy-13623"]
    ex, p = f.get("executed_rub"), f.get("price_rub")
    if ex is not None and p is not None and ex > 0:
        return "partially", f"Принято по актам {fmt(ex)} ₽ из {fmt(p)} ₽: исполненная часть из расчёта исключается, остаток {fmt(max(p - ex, 0))} ₽ учитывается. Договор не снимается с учёта до итогового акта.", ["grk-55.13-7", "gk-753"]
    return "needs_facts", "Актов приёмки (КС-2, КС-11, итоговый акт) не представлено: без них исполнение не подтверждено.", ["grk-55.13-7", "gk-753"]


def t_terminated(f):
    if f.get("termination_document"):
        return "founded", f"Расторжение подтверждено ({f['termination_document']}): с даты расторжения обязательств нет, принятые до этого работы — по актам. О расторжении уведомляется СРО.", ["grk-55.8-4", "grk-55.13-7"]
    return "needs_facts", "Документ о расторжении (соглашение, уведомление об одностороннем отказе с датой) не представлен.", ["grk-55.8-4"]
...
def t_design_threshold(f):
    if f.get("contract_kind") in ("design", "survey"):
        return "unfounded", "Порог 10 млн ₽ установлен только для строительного подряда. Для подготовки проектной документации и инженерных изысканий суммового порога нет.", ["grk-48-4", "grk-47-2", "grk-52-2.1"]
...
def t_before_2026(f):
    if f.get("executed_before_2026_03_01") and f.get("has_final_act"):
        return "founded", "Договор полностью исполнен до 1 марта 2026 года: под уведомление по 309-ФЗ не попадает, обязательств по нему нет.", ["fz-309", "grk-55.13-7"]
    return "unfounded", "Дата заключения значения не имеет: под уведомление и учёт попадают все договоры, по которым на 1 марта 2026 года оставались неисполненные обязательства.", ["fz-309", "grk-55.8-4", "grk-55.13-7"]
...
def t_not_member(f):
    return "unfounded", "Совокупный размер обязательств считается по действующим договорам члена СРО на дату расчёта независимо от даты вступления. Выполнение работ без членства, когда оно требовалось, — отдельное нарушение, а не основание исключить договор.", ["grk-55.8-3", "grk-55.13-7", "grk-52-2.1"]
