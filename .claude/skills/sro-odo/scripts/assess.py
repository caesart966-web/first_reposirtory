...
class Assessor:
    ...
    def assess(self, card: dict, find_cases: bool = True) -> dict:
        ...
        if kind in ("construction", "demolition"):
            if price is None:
                ...
                findings.append(self.F(4, "Порог по одному договору", "no",
                    ...
                    + ((f" Начальная (максимальная) цена закупки {money(c['nmck_rub'])} ₽ была выше порога, а цена договора после торгов — ниже: заказчик правомерно "
                        "требовал членство на торгах (ст. 31 44-ФЗ), но порог ч. 2.1 ст. 52 применяется к цене договора. Единой практики нет — решение за проверяющим.")
                       if (c.get("nmck_rub") or 0) > thr else "")
                ...
                if (c.get("nmck_rub") or 0) > thr:
                    missing.append("решение проверяющего: начальная цена закупки выше порога, цена договора ниже — спорный случай")
            else:
        ...
        if status == "completed" and c.get("has_final_act"):
            ...
            sro_cite = ["sro-odo-1.2"] if self.policy.get("sro") else []
            if proc == "competitive" and c.get("procurement_law") == "voluntary_tender" and self.policy.get("voluntary_tender", "direct") == "direct":
                odo_note = "voluntary_tender"
                findings.append(self.F(5, "Совокупный размер обязательств", "no",
                    "Договор заключён по итогам тендера, но проведение торгов для него законом не предписано: закупка вне 44-ФЗ и 223-ФЗ, заказчик не обязан был "
                    "их проводить. Конкурентным способом по смыслу ч. 1 ст. 60.1 ГрК РФ (то же — п. 1.2 Положения СРО о КФ ОДО) считаются только закупки по 44-ФЗ, "
                    "223-ФЗ и торги, обязательные по закону. Для КФ ОДО такой договор равен прямому: в совокупный размер не входит, уведомляется в СРО (ч. 4 ст. 55.8)."
                    + (f" Основание: {c['procurement_basis']}." if c.get("procurement_basis") else ""),
                    ["grk-60.1-1", "grk-55.8-3", "grk-55.8-4"] + sro_cite))
                hard_no_odo = True
            elif proc == "competitive" and "below_threshold" in hard_no and self.policy.get("below_threshold_competitive") != "include":
                findings.append(self.F(5, "Совокупный размер обязательств", "no",
                ...
                findings.append(self.F(5, "Совокупный размер обязательств", "yes",
                    ...
                    "исполненная по актам часть исключается (ч. 7 ст. 55.13)."
                    + {"44-fz": " Торги по 44-ФЗ — конкурентный способ по прямому указанию ст. 60.1 ГрК РФ.",
                       "223-fz": " Закупка по 223-ФЗ — конкурентный способ по прямому указанию ст. 60.1 ГрК РФ.",
                       "mandatory_tender": " Торги обязательны по закону — конкурентный способ по ст. 60.1 ГрК РФ.",
                       "unknown": " Чем предписаны торги (44-ФЗ, 223-ФЗ, иная обязательная процедура) — не установлено; если тендер был добровольным, договор для КФ ОДО равен прямому."}.get(c.get("procurement_law") or "unknown", "")
                    + (" То же определение конкурентного способа — в п. 1.2 Положения СРО о КФ ОДО." if sro_cite else "")
                    + (f" Основание: {c['procurement_basis']}." if c.get("procurement_basis") else ""),
                    ["grk-55.8-3", "grk-60.1-1", "grk-55.13-7", "grk-55.16-13" if sro_kind == "build" else "grk-55.16-11", "fz-44", "fz-223"] + sro_cite))
            elif proc == "direct":
                ...
                findings.append(self.F(5, "Совокупный размер обязательств", "needs_facts",
                    "Способ заключения договора не установлен: торги по 44-ФЗ или 223-ФЗ, иные обязательные по закону торги — или прямой договор. "
                    "Добровольный тендер заказчика конкурентным способом не считается (ч. 1 ст. 60.1 ГрК РФ).",
                    ["grk-55.8-3", "grk-60.1-1", "fz-309"] + sro_cite, ["способ заключения договора"]))
    ...
    def _summary(self, card, membership, counts, sro_kind, price, share, remaining, hard_no, odo_conditional):
        ...
        elif counts == "no" and c.get("procurement_law") == "voluntary_tender":
            s += (" В совокупный размер обязательств по КФ ОДО он не входит: тендер заказчика не был обязательным по закону, а конкурентным способом "
                  "считаются только 44-ФЗ, 223-ФЗ и обязательные торги (ч. 1 ст. 60.1 ГрК РФ); уведомлению в СРО подлежит (ч. 4 ст. 55.8).")
        elif counts == "no":
