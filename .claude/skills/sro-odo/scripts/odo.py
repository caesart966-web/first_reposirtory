...
def compute(reg: dict, levels: dict) -> dict:
    ...
    return {
        ...
        "method_notes": [
          ...
          + (["Положение СРО о контроле: проверка соответствия совокупного размера уровню — не реже раза в год (п. 3.2), по ст. 55.13 ГрК РФ и порядку уведомления Минстроя (п. 7.2); "
              "договоры сноса считаются наравне со строительным подрядом (п. 2.1); член вправе возразить на акт в 15 календарных дней (раздел 12)."]
             if policy.get("sro") and policy["sro"].get("control_regulation") else []),
