...
class TestПаузаПередПовтором(unittest.TestCase):
    """Пауза должна зависеть от того, чем именно отказал реестр."""

    def test_пятисотка_дешёвая(self):
        # У этого реестра 500 случайная: тот же запрос через секунду проходит.
        # Пара 2/4/8/16 съела полтора часа на прогоне 1892 ИНН по НОСТРОЮ
        self.assertEqual([нопориз._пауза_перед_повтором(500, n) for n in (1, 2, 3, 4)],
                         [1, 2, 3, 4])

    def test_просьбу_сбавить_темп_слушаем(self):
        # 429 и 503 — это прямая просьба сервера; торопиться здесь значит
        # получить отказ на весь оставшийся прогон
        self.assertEqual(нопориз._пауза_перед_повтором(429, 4), 16)
        self.assertEqual(нопориз._пауза_перед_повтором(503, 4), 16)

    def test_длинная_пауза_ограничена(self):
        self.assertEqual(нопориз._пауза_перед_повтором(429, 10), 30)

    def test_обрыв_сети_ждём_по_настоящему(self):
        # status=None — исключение requests: сеть пропала, секунда не поможет
        self.assertEqual(нопориз._пауза_перед_повтором(None, 3), 8)
