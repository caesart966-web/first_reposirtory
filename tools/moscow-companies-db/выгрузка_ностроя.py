...
def членства_строителя(payload, inn: str) -> tuple:
    """Строительные членства компании: (действующее, прекращённое).

    Каждое — (дата вступления, СРО, номер СРО, дата прекращения) или None.
    Различать эти два случая обязательно: «в реестре нет вовсе» и «была,
    но исключена» — разные вещи и для проверки полноты данных, и для
    продаж. Раньше оба писались как «нет», и по 53 компаниям было
    не понять, потеряли мы их или они честно выбыли.

    У действующего берётся самое раннее вступление, у прекращённого —
    самый поздний выход: сколько прошло с последнего исключения, столько
    осталось от годичного запрета на вступление в другую СРО.

    Поиск идёт по строке, поэтому в ответ попадают и чужие компании —
    сверяем ИНН каждой записи.
    """
    действующее = прекращённое = None
    for record in records_of(payload):
        ...
        if started is None:
            continue
        stopped = _date_by_hints(record, _STOP_KEY_HINTS)
        сведения = (started, sro_name(record), sro_registration_number(record), stopped)
        if is_former(record):
            if прекращённое is None or _выход(сведения) > _выход(прекращённое):
                прекращённое = сведения
        elif действующее is None or started < действующее[0]:
            действующее = сведения
    return действующее, прекращённое


def _выход(сведения: tuple) -> date:
    """Когда компания вышла из СРО. Даты прекращения в реестре есть не
    всегда — тогда сравниваем по дате вступления, иначе запись без даты
    выхода всегда проигрывала бы записи с датой."""
    return сведения[3] or сведения[0]
...
def cmd_check(args) -> int:
    ...
    if args.resume and progress_path.exists():
        ...
        # Прогресс прежних версий не знал о прекращённых членствах: там
        # «нет в реестре» стояло и у тех, кого исключили. Такие ИНН
        # спрашиваем заново, иначе --resume законсервировал бы ошибку
        устарели = [и for и, в in готово.items()
                    if "Было строительное (прекращено)" not in в]
        for и in устарели:
            del готово[и]
        print(f"[нострой] продолжаю, уже проверено {len(готово)}")
        if устарели:
            print(f"[нострой] {len(устарели)} ИНН проверены старой версией "
                  f"(без прекращённых членств) — спрошу заново")
    ...
    try:
        for номер, инн in enumerate(к_проверке, 1):
            ...
            if payload is None:
                ...
                continue
            действующее, прекращённое = членства_строителя(payload, инн)
            готово[инн] = {
                "Строительное: дата вступления": действующее[0].strftime("%d.%m.%Y") if действующее else "",
                "Строительное: СРО": действующее[1] if действующее else "",
                "Строительное: номер СРО": действующее[2] if действующее else "",
                "Было строительное (прекращено)": прекращённое[1] if прекращённое else "",
                "Дата прекращения": прекращённое[3].strftime("%d.%m.%Y")
                                    if прекращённое and прекращённое[3] else "",
            }
            if номер % 25 == 0:
    ...
    новые = ["Строительное: дата вступления", "Строительное: СРО",
             "Строительное: номер СРО", "Было строительное (прекращено)",
             "Дата прекращения", "В НОСТРОЙ"]
    columns = header + [c for c in новые if c not in header]
    состоят = выбыли = нет = не_проверены = 0
    for row, инн in zip(rows, инны):
        ...
        elif найдено.get("Было строительное (прекращено)"):
            row["В НОСТРОЙ"] = "членство прекращено"; выбыли += 1
        else:
            row["В НОСТРОЙ"] = "нет в реестре"; нет += 1
    ...
    print(f"[нострой]   членство прекращено:        {выбыли}")
    print(f"[нострой]   нет в реестре вовсе:        {нет}")
    if не_проверены:
...
def cmd_sample(args) -> int:
    ...
    if args.инн:
        инн = норм_инн(args.инн)
        payload = fetch_by_inn(session, инн)
        записи = [r for r in records_of(payload or {}) if норм_инн(r.get("inn")) == инн]
        if not записи:
            print(f"По ИНН {инн} реестр ничего не отдал.", file=sys.stderr)
            return 1
        records = записи
        source = f"ИНН {инн}, записей {len(records)}"
    elif args.sro_id:
