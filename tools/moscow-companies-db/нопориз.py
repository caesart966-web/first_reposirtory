...
def членства(payload: Any, inn: str) -> tuple:
    """Членства компании: (проект, изыск, бывший проект, бывший изыск).

    Действующее — (дата вступления, СРО), самое раннее. Прекращённое —
    (дата вступления, СРО, дата выхода), самое позднее по выходу: от него
    отсчитывается годичный запрет на вступление в другую СРО.

    Различать «в реестре нет вовсе» и «была, но выбыла» обязательно.
    Раньше исключённые записи просто пропускались, и оба случая выглядели
    одинаково — а это разные вещи и для проверки полноты данных,
    и для продаж.
    ...
    """
    проект = изыск = бывший_проект = бывший_изыск = None
    for record in records_of(payload):
        ...
        имя = sro_name(record)
        if is_former(record):
            сведения = (started, имя, stop_date(record))
            if DESIGN in вид and (бывший_проект is None
                                  or _выход(сведения) > _выход(бывший_проект)):
                бывший_проект = сведения
            if SURVEY in вид and (бывший_изыск is None
                                  or _выход(сведения) > _выход(бывший_изыск)):
                бывший_изыск = сведения
            continue
        if DESIGN in вид and (проект is None or started < проект[0]):
            проект = (started, имя)
        if SURVEY in вид and (изыск is None or started < изыск[0]):
            изыск = (started, имя)
    return проект, изыск, бывший_проект, бывший_изыск


def stop_date(record: dict) -> date | None:
    """Дата выхода из СРО: самая ранняя из найденных."""
    found = _dates_by_hints(record, _STOP_KEY_HINTS)
    return min(d for d, _k in found) if found else None


def _выход(сведения: tuple) -> date:
    """Когда компания вышла. Даты прекращения в реестре есть не всегда —
    тогда сравниваем по дате вступления, иначе запись без даты выхода
    всегда проигрывала бы записи с датой."""
    return сведения[2] or сведения[0]


def _прекращено(сведения: tuple | None) -> str:
    """Прекращённое членство одной строкой: «СРО — до 10.09.2025»."""
    if сведения is None:
        return ""
    имя, вышел = сведения[1], сведения[2]
    return f"{имя} — до {вышел:%d.%m.%Y}" if вышел else имя
...
def cmd_match(args) -> int:
    ...
    new_cols = ["Проектирование: дата вступления", "Проектирование: СРО",
                ...
                "Было проектное (прекращено)", "Было изыскательское (прекращено)",
                "В НОПРИЗ"]
    columns = header + [c for c in new_cols if c not in header]

    both = only_design = only_survey = neither = no_inn = выбыли = 0
    for row in rows:
...
def cmd_recheck(args) -> int:
    ...
    if args.resume and progress_path.exists():
        ...
        # «нет» стояло и у тех, кого исключили. Такие ИНН спрашиваем
        # заново, иначе --resume законсервировал бы ошибку
        устарели = [и for и, в in готово.items()
                    if "Было проектное (прекращено)" not in в]
        for и in устарели:
            del готово[и]
        print(f"[проверка] продолжаю, уже проверено {len(готово)}")
        if устарели:
            print(f"[проверка] {len(устарели)} ИНН проверены старой версией "
        ...
        for номер, инн in enumerate(к_проверке, 1):
            ...
            if payload is None:
                ...
                print(f"[проверка] ИНН {инн} не проверился — пропускаю",
                      file=sys.stderr)
                continue
            п, и, бп, би = членства(payload, инн)
            готово[инн] = {
                ...
                "Было проектное (прекращено)": _прекращено(бп),
                "Было изыскательское (прекращено)": _прекращено(би),
            }
            if номер % 25 == 0:
    ...
    for row, инн in zip(rows, инны):
        ...
        elif найдено.get("Было проектное (прекращено)") or \
                найдено.get("Было изыскательское (прекращено)"):
            row["В НОПРИЗ"] = "членство прекращено"; выбыли += 1
    ...
    print(f"[проверка]   членство прекращено:            {выбыли}")
